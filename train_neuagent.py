...
def init_params():
    ...
    params['b'] = 0.1 * np.ones((mul * prm.dim_proj,)).astype(config.floatX) # bias
    ...
    for i in range(prm.n_doc_layers_nav):
        ...
        params['b_U_L' + str(i)] = 0.1 * np.ones((prm.dim_proj,)).astype(config.floatX) # bias
    ...
    if prm.lambda_scoring > 0.:
        for i in range(prm.n_doc_layers_final):
            if i == 0:
                in_dim = prm.dim_emb
            else:
                in_dim = prm.dim_proj
            params['Q' + str(i)] = 0.01 * np.random.randn(in_dim, prm.dim_proj).astype(config.floatX) # questions
            params['P' + str(i)] = 0.01 * np.random.randn(in_dim, prm.dim_proj).astype(config.floatX) # pages
            params['bq' + str(i)] = np.zeros((prm.dim_proj,)).astype(config.floatX) # bias
            params['bp' + str(i)] = np.zeros((prm.dim_proj,)).astype(config.floatX) # bias

        ns = [prm.dim_proj] + prm.scoring_layers_final + [1]
        for i in range(len(ns)-1):
            params['U_Z'+ str(i)] = 0.01 * np.random.randn(ns[i], ns[i+1]).astype(config.floatX) # score    
            params['b_U_Z'+str(i)] = np.zeros((ns[i+1],)).astype(config.floatX) # bias
    
    if prm.att_query:
        params['Ww_att_q'] = 0.01 * np.random.randn(prm.dim_emb, prm.att_window).astype(config.floatX) # query
        params['Wq_att_q'] = 0.01 * np.random.randn(prm.dim_emb, prm.dim_proj).astype(config.floatX) # query
        params['Wh_att_q'] = 0.01 * np.random.randn(prm.dim_proj, prm.dim_proj).astype(config.floatX) # hidden state
        params['Wl_att_q'] = 0.01 * np.random.randn(prm.dim_emb, prm.dim_proj).astype(config.floatX) # link embedding
        params['bq_att_q'] = 0.1 * np.ones((prm.dim_proj,)).astype(config.floatX) # bias
    ...
    if prm.att_doc:
        ...
        params['Wl_att_d'] = 0.01 * np.random.randn(prm.dim_emb, prm.dim_proj).astype(config.floatX) # link embedding
        params['bq_att_d'] = 0.1 * np.ones((prm.dim_proj,)).astype(config.floatX) # bias
    ...
    # don't compute gradients for the last scoring layer when not using supervised learning
    # TODO: remove this in the future..
    if prm.learning.lower() != 'supervised':
        for i in range(prm.n_doc_layers_final):
            exclude_params['Q'+str(i)] = True
            exclude_params['P'+str(i)] = True
            exclude_params['bq'+str(i)] = True
            exclude_params['bp'+str(i)] = True

        for i in range(len(prm.scoring_layers_final)+1):
            exclude_params['U_Z'+str(i)] = True
            exclude_params['b_U_Z'+str(i)] = True
...
def val(q_a, q_m, h_, l_a_, c_, m_, L_a, L_m, tparams_v, tparams, k_beam, n_samples, uidx, is_train, trng):
    ...
    if prm.att_query:
        ...
        q_aw = q_aw_[:,:-(prm.att_window-1),:] * fparams('Ww_att_q')[:,0]
        # convolution embeddings
        for i in range(1,prm.att_window):
            q_aw += q_aw_[:,i:q_aw_.shape[1]-(prm.att_window-1)+i,:] * fparams('Ww_att_q')[:,i]
        q_aw += fparams('bw_att_q')
    ...
    for i in range(prm.n_rnn_layers):
        if i == 0:
            a = tensor.dot(q_at, fparams('E_Q'))
            if prm.dropout > 0:
                a = dropout_layer(a, is_train, trng)

            b = tensor.dot(l_a_, fparams('E_L'))
            if prm.dropout > 0:
                b = dropout_layer(b, is_train, trng)

            preact = a + b
            preact += tensor.dot(h_[i], fparams('U_I'))
            preact += fparams('b')
        else:
            a = tensor.dot(q_at, fparams('E_Q' + str(i)))
            if prm.dropout > 0:
                a = dropout_layer(a, is_train, trng)

            b = tensor.dot(l_a_, fparams('E_L' + str(i)))
            if prm.dropout > 0:
                b = dropout_layer(b, is_train, trng)

            preact = a + b
            preact += tensor.dot(h[i-1], fparams('U_H' + str(i)))
            preact += tensor.dot(h_[i], fparams('U_I' + str(i)))
            preact += fparams('b' + str(i))
    ...
    if prm.att_doc:
        ...
        L_aw = L_aw_[:,:,:-(prm.att_window-1),:] * fparams('Ww_att_d')[:,0]
        # Convolution embeddings
        for i in range(1,prm.att_window):
            L_aw += L_aw_[:,:,i:L_aw_.shape[2]-(prm.att_window-1)+i,:] * fparams('Ww_att_d')[:,i]
        L_aw += fparams('bw_att_d')
    ...
    for i in range(prm.n_doc_layers_nav):
        if i == 0:
            ...
            i_ = str(i)
        L_as2 = tensor.tanh(tensor.dot(L_as2, fparams('U_L'+i_)) + fparams('b_U_L'+i_))
    ...
    # normalize. Avoid NaN by using float64 instead of float32.
    res = (res - res.mean(1, keepdims=True)).astype('float64') / tensor.maximum(1., res.std(1, keepdims=True)).astype('float64')
...
def ff(q, q_m, k_beam, trng, is_train, options, uidx, tparams, mixer, sup, root_pages, max_hops, acts_p, rl_idx=None, get_links=None):
    ...
    # append vector of zeros for words whose index = -1.
    W_ = tensor.concatenate([tparams['W'], tensor.alloc(0., 1, prm.dim_emb)], axis=0)
    ...
    def _step(it, act_p, p_, m_, l_a_, h_, c_, q_a, pr_all, W_, k_beam, uidx, is_train, sup, mixer, *tparams_v):
        ...
        dist, h, c, L_as, L_ms = val(q_a, q_m, h_, l_a_, c_, m_, L_a, L_m, tparams_v, tparams, k_beam, n_samples, uidx, is_train, trng)
            ...
            if rl_idx: #if this is the replay memory pass, just use the q-value function
                ...
                if prm.act_sel.lower() == 'epsilon-greedy':
                    ...
                    res_pre = tensor.eq(it, 0.).astype('int32') * dist[:,:n_links].argsort(axis=1)[:, -k_beam:].flatten().astype("int32") \
                            + tensor.neq(it, 0.).astype('int32') * dist.argsort(axis=1)[:, -k_beam:].reshape((n_samples * k_beam,)).astype("int32")
                    ...
                    res_ = tensor.eq(it, 0.).astype('int32') * lp_[:,:n_links].argsort(axis=1)[:, -k_beam:].flatten().astype("int32") \
                         + tensor.neq(it, 0.).astype('int32') * lp_.argsort(axis=1)[:, -k_beam:].reshape((n_samples * k_beam,)).astype("int32")
        ...
        return p, m, l_a, h, c, l_prob, l_ent, cost_p, l_idx, dist_

    #get embeddings for the queries
    q_a = tparams['W'][q.flatten()].reshape((q.shape[0], q.shape[1], prm.dim_emb)) * q_m[:,:,None]

    init = q_a.sum(1) / tensor.maximum(1., q_m.sum(1, keepdims=True))
    init = tensor.extra_ops.repeat(init, k_beam, axis=0)
    ...
    if prm.init_zero:
        l_a_init = tensor.alloc(0., k_beam * n_samples, prm.dim_emb)
        h_init = tensor.alloc(0., prm.n_rnn_layers, k_beam * n_samples, prm.dim_proj)
        h_init = tensor.patternbroadcast(h_init, (False,False,False))
        c_init = tensor.alloc(0., prm.n_rnn_layers, k_beam * n_samples, prm.dim_proj)
        c_init = tensor.patternbroadcast(c_init, (False,False,False))
    else:
        #initialize states with query embeddings. This is important for Q-learning.
        l_a_init = 1. * init
        init = tensor.dot(init, tparams['E_Q'][:,:prm.dim_proj])
        init = theano.gradient.disconnected_grad(init)
        init = tensor.extra_ops.repeat(init, prm.n_rnn_layers, axis=0)
        init = init.reshape((prm.n_rnn_layers, -1, init.shape[1]))
        init = tensor.patternbroadcast(init, (False,False,False))
        h_init = 1. * init
        c_init = 1. * init

    pr_all = tensor.alloc(1., max_hops+1, k_beam * n_samples)

    (pages_idx, mask, l_a, h, _, l_prob, l_ent, cost_p, l_idx, dist), scan_updates = theano.scan(_step,
                                sequences=[tensor.arange(max_hops+1), acts_p],
                                outputs_info=[root_pages_, #page idx
                                              tensor.alloc(0., k_beam * n_samples),  # mask
                                              l_a_init, h_init, c_init,
                                              None, # l_prob
                                              None,  # l_ent
                                              None,  # cost_p
                                              None,  # l_idx
    ...
    return (pages_idx, mask, l_a, q_a, h[:,-1,:,:], l_prob, l_ent, cost_p, root_pages_, l_idx, dist), scan_updates, get_links
...
def build_model(tparams, tparams_next, baseline_vars, options):
    ...
    (pages_idx, mask, l_a, q_a, h, l_prob, l_ent, cost_p, root_pages_, l_idx, dist), scan_updates_a, _ = \
    ...
    j = (tensor.minimum(mask.shape[0] - 1, mask.sum(axis=0))).astype("int32") 
    pps = pages_idx_.T
    pps = pps[tensor.arange(pps.shape[0]), j]

    R, best_answer = get_sent(q, q_m, pps, k_beam)
    ...
    pages_idxT = pages_idx.T
    pages_idxT = pages_idxT.reshape((q.shape[0],-1))
    ...
    if prm.lambda_scoring > 0.:
        ...
        l_aT_ = l_aT
        for i in range(prm.n_doc_layers_final):
            q_a_ = tensor.tanh(tensor.dot(q_a_, tparams['Q'+str(i)]) + tparams['bq'+str(i)])
            l_aT_ = tensor.tanh(tensor.dot(l_aT_, tparams['P'+str(i)]) + tparams['bp'+str(i)])

        z = tensor.dot(q_a_[:, None, :] * l_aT_, tparams['U_Z0']) + tparams['b_U_Z0']

        for i in range(1,len(prm.scoring_layers_final)+1):
            z = tensor.tanh(z) # tanh here instead after the dot product makes no tanh in the last layer.
            z = tensor.dot(z, tparams['U_Z' + str(i)]) + tparams['b_U_Z' + str(i)]

        distZ = softmax_mask(z[:,:,0], maskT.reshape((q.shape[0],-1)))

        # select an document that has the highest probability in the distribution

        best_doc = pages_idxT[tensor.arange(pages_idxT.shape[0]), distZ.argmax(axis=1)]
    else:
        best_doc = pages_idxT[:, 0]

    f_pred = theano.function([q, q_m, root_pages, acts_p, uidx], \
                             [best_doc, best_answer, R, pages_idx, pps, j], \
    ...
    if prm.learning.lower() == 'supervised':
        ...
        if prm.lambda_scoring > 0.:
            cost += prm.lambda_scoring * (-tensor.log(distZ[tensor.arange(distZ.shape[0]), a] + off)).mean()
            cost += prm.lambda_scoring * prm.erate * -(distZ * tensor.log(distZ + off)).sum(1).mean()
    ...
    out = [cost, R, l_idx, pages_idx, best_doc ,best_answer, maskT, dist]
...
def pred_error(f_pred, queries, actions, candidates, options, iterator, verbose=False):
    ...
    recall_last2 = 0. # document recall for last 2 pages before the stop action.
    recall_all = 0. # document recall for all pages visited.
    ii_exclusive = 0. # document recall of the questions that can only be answered by the inverted-index system.
    nn_exclusive = 0. # document recall of the questions that can only be answered by NeuAgent.
    nn_ii = 0. # document recall of the questions that can be answered by NeuAgent and the inverted-index sytem.
    uidx = -1
    ...
    for _, valid_index in iterator:
        ...
        best_doc, best_answer, R, pages_idx, pps, js = f_pred(q_i, q_m, root_pages, acts_p, uidx)
        ...
        selected_docs = pps.reshape((len(valid_index),prm.k))
        selected_docs_last2 = pages_idx[js-2,np.arange(pages_idx.shape[1])].reshape((len(valid_index),prm.k))
        ...
        for j in range(len(valid_index)):
            ...
            for correct_doc in correct_docs:
                ...
                # doc recall for the last 2 pages before stop action
                recall_last2 += match.astype('int32').sum()
                recall_last2 += (correct_doc == selected_docs_last2[j]).any().astype('int32').sum()
            
                acc += (correct_doc == best_doc[j]).astype('int32').sum()

                if correct_doc in cands[j]:
                    if match:
                        nn_ii += 1.
                    else:
                        ii_exclusive += 1.
    ...
    acc = acc / n
    recall = recall / ns
    recall_last2 = recall_last2 / ns
    recall_all = recall_all / ns
    nn_exclusive = nn_exclusive / ns
    ii_exclusive = ii_exclusive / ns
    nn_ii = nn_ii / ns

    return valid_R, acc, recall, recall_last2, recall_all, nn_exclusive, ii_exclusive, nn_ii
...
def train_lstm():
    ...
    try:
        for eidx in xrange(prm.max_epochs):
            ...
            for _, train_index in kf:
                ...
                # select the first path of each training example
                actions = [a_train[t][0] for t in train_index]
                ...
                if np.mod(uidx, validFreq) == 0 or uidx == 1:
                    ...
                    train_R, train_acc, train_recall, train_recall_last2, train_recall_all, \
                        train_nn_exclusive, train_ii_exclusive, train_nn_ii \
                        = pred_error(f_pred, q_train, a_train, c_train, options, kf_train)

                    print '\nComputing Error Validation Set'
                    valid_R, valid_acc, valid_recall, valid_recall_last2, valid_recall_all, \
                        valid_nn_exclusive, valid_ii_exclusive, valid_nn_ii \
                         = pred_error(f_pred, q_valid, a_valid, c_valid, options, kf_valid)

                    print '\nComputing Error Test Set'
                    test_R, test_acc, test_recall, test_recall_last2, test_recall_all, \
                        test_nn_exclusive, test_ii_exclusive, test_nn_ii \
                    ...
                    print 'Recall', prm.k, 'Train', train_recall, ' Valid', valid_recall, ' Test',test_recall
                    print 'Recall', 2 * prm.k, 'Train Last 2 Pages', train_recall_last2, ' Valid', valid_recall_last2, ' Test',test_recall_last2
                    print 'Recall', prm.max_hops_pred * prm.k, 'All Visited Train', train_recall_all, ' Valid', valid_recall_all, ' Test',test_recall_all
                    print 'Recall NeuAgent Exclusive', train_nn_exclusive, ' Valid', valid_nn_exclusive, ' Test', test_nn_exclusive
                    print 'Recall Inverted-Index Exclusive', train_ii_exclusive, ' Valid', valid_ii_exclusive, ' Test', test_ii_exclusive
                    print 'Recall NeuAgent and Inverted-Index', train_nn_ii, ' Valid', valid_nn_ii, ' Test', test_nn_ii
    ...
    train_R, train_acc, train_recall, train_recall_last2, train_recall_all, \
        train_nn_exclusive, train_ii_exclusive, train_nn_ii \
        = pred_error(f_pred, q_train, a_train, c_train, options, kf_train_sorted)
    ...
    if prm.saveto:
        np.savez(prm.saveto, train_R=train_R,
                    valid_R=valid_R, test_R=test_R,
